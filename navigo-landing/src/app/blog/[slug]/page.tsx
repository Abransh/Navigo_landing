...
export async function generateMetadata({ params } : BlogProp) {
  ...
  return {
    ...
    openGraph: {
      title: post.title,
      description: post.excerpt,
      url: `https://www.trynavigo.com/blog/${post.slug}`,
      siteName: "Navigo",
      images: [
        {
          url: post.coverImage,
...
export default async function BlogPostPage({ params } : BlogProp ) {
  const { slug } = await params;
